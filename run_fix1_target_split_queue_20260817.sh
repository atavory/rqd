...
OUT=${OUT:-$ROOT/results/fix1_target_split_confirm_20260817}
...
try:
    ...
    raise SystemExit(1)
rows = payload.get("target_item_split_rows", [])
...
run_one() {
  ...
  local fd=$4
  local seed=$5
  local output="$OUT/downstream_${label}_${arch}_fd${fd}_seed${seed}_fix1_new.json"
  ...
  wait_for_existing
  log "run FIX-1 label=$label arch=$arch fd=$fd seed=$seed split=$FIX1_TARGET_SPLITS"
...
log "FIX-1 target-split queue start OUT=$OUT DEVICE=$DEVICE"
...
summarize >>"$LOG" 2>&1
log "FIX-1 target-split queue done"
